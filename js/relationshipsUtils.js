 ...
 */
 ...
 var FRIEND_COLOR = '#93c6b1';
 var FRIEND_SECONDARY_COLOR = '#def7ed';
 
 var FAMILY_COLOR = '#ff9bb2';
 var FAMILY_SECONDARY_COLOR = '#ffe8ed';
 
 var ACQ_COLOR = '#fff09b';
 var ACQ_SECONDARY_COLOR = '#fffbe8';
...
var GE_COLOR = '#DAA520';
 ...
 var SEARCHED_NODE_COLOR = '#701313';
 
 //values for sizing elements
 var MAGNIFYING_RATIO = 1.5;
 var MARGIN = 20;
...
var USER_MENU_WIDTH_HIDE = '160px';
 ...
 function selectNode(node) {
    node.attr('r', function(d) { return d.Radius});
     ...
     if(parent._groups[0][0].childElementCount < 2) {
        ...
        var x = node.attr('cx') - r; 
        var y = node.attr('cy') - r;
        ...
        // var x = group.append('body')

        var f = group.append('foreignObject')
            .attr('x', x - 30)
 ...
 function showSummary(node) {
    ...
    d3.select('#BiographySummary').append('text').html(handleBiographyText(data['Biography'])); 
    if(data.Image) {
        document.getElementById('BiographyPhoto').src = "images/png/" + data.Image;
    }
    if(data['Letters']) {
        d3.select('#BiographyLetters').append('text').html("<u>Letters from <i>George Eliot letters</i>, vols 1-9, edited by Gordon Haight (Yale UP, 1954-1978):</u></br>")
 ...
 function filterInput() {
     ...
     if(searchTerm.length > 4) {
         var indicies = getIndiciesOfSearchTerm(searchTerm);
 
         indicies.forEach(function(index) {
             var nodeId = people[index].id;
 ...
 function getIndiciesOfSearchTerm(searchTerm) {
     ...
     people.forEach(function(person) {
         ...
         if(person.name.toLowerCase().includes(searchTerm)) {
             index = people.map(function(e) { return e.name; }).indexOf(person.name);
             indicies.push(index);
         }
     });
     return indicies;
 ...
 function selectSearchedNode(node) {
     ...
     //values from node
     var r = node._groups[0][0].firstChild.r.animVal.value;
     //offset r from x and y to prevent overlap with image
     var x = data['x'];
     var y = data['y'];
